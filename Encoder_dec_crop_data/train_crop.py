...
import tokenizer
from custom_dataloader import ID_card_DataLoader, All_ID_card_DataLoader, final_dataLoader
from torchvision import transforms
from torch.utils.data import DataLoader
import numpy as np
import tqdm
from Networks_crop import Encoder, AttnDecoderRNN, EncoderRNN
...
def train(args, encoderCNN, encoderRNN, decoderRNN, encoderCNN_optimizer, encoderRNN_optimizer
          , decoderRNN_optimizer, device, dataloader1, epoch, start, criterion_ctc, batch_size):
    encoderCNN.train()
    encoderRNN.train()
    decoderRNN.train()
    batch_loss = 0
    ...
    for batch_idx, data in enumerate(dataloader1):
        ...
        try:
            ...
            images, target, label_type = data[0].to(device), data[1].to(device), data[2].to(device)
            ...
            encoderRNN_init_hidden = feature_vector
            ...
            input_lengths = torch.full((batch_size,), 35)  # All logits sequences have length 160
            target_lengths = torch.full((batch_size,), 35)  # All target sequences have length 160
    ...
    print('Epoch {} CTC TRAINING COMPLETED. final losses:\nLoss ctc: {:.6f}'.format(epoch, tot_loss_ctc/ (len(dataloader1)*batch_size)))
    torch.cuda.empty_cache()

    return tot_loss_ctc/ (len(dataloader1)*batch_size)


def evaluation(args, val_loader, encoderCNN, encoderRNN, decoderRNN, encoderCNN_optimizer
               , encoderRNN_optimizer, decoderRNN_optimizer, device
               , epoch, batch_size, criterion_ctc, ctc_loss_min):
    ...
    with torch.no_grad():
        ...
        for batch_idx, data in enumerate(val_loader):
            try:
                images, target, label_type, character_label = data[0].to(device), data[1].to(device), data[2].to(device), data[3]
                # images, target = images.to(device), target
                # target_encoder = torch.cat((target['person_coordinate'], target['rotation'].unsqueeze(1), target['scale'].unsqueeze(1), target['transport']), dim=1).to(device).float()
                target_decoder = target
                feature_map, feature_vector = encoderCNN(images)
                # features = feature_extractor(images)
                encoderRNN_init_hidden = feature_vector
                # torch.cat((feature_vector.unsqueeze(0), label_type.unsqueeze(0)), dim=2)
                encoderRNN_output, decoderRNN_init_hidden = encoderRNN(
                    feature_map.view(batch_size, feature_map.size()[1],
                ...
                for i in range(batch_size):
                    zero_numbers = (target_decoder[i, :] == 127).sum()
                    target_lengths[i] = target_lengths[i] - zero_numbers
                    print(f'target : \t {character_label[i]}')
                    decoded_words = []
                ...
                print(f'ctc mean loss is {loss_ctc.item()}')
            except Exception as e:
        ...
        torch.cuda.empty_cache()
        if args.save_model and (loss_ctc < ctc_loss_min):
            filename = ('/home/kasra/PycharmProjects/Larkimas/Encoder_dec_crop_data/model_checkpoints'
                        '/decoder: epoch: {0} ctc_l:{1}.pt').format(epoch, np.round(loss_ctc.cpu(), 3))
            torch.save({'epoch': epoch, 'state_dict encoderCNN': encoderCNN.state_dict(),
                        ...
                        'encoderCNN_optimizer': encoderCNN_optimizer.state_dict(), 'encoderRNN_optimizer': encoderRNN_optimizer.state_dict()
                        ,'decoderRNN_optimizer': decoderRNN_optimizer.state_dict()}, filename)
            print('decoder model has been saved in {}'.format(filename))
            ctc_loss_min = loss_ctc
...
def main():
    ...
    parser.add_argument('--valid-batch-size', type=int, default=110, metavar='N',
...
# '/home/kasra/PycharmProjects/Larkimas/model_checkpoints/encoder_epoch_1_ctc_loss_4.3.pt'
    ...
    encoderCNN = Encoder(base_filters=16, adaptive_pool_size=(10, 30)).to(device)
    ...
    # , weight_decay=4e-4
    scheduler_enc = StepLR(encoderCNN_optimizer, step_size=3, gamma=args.gamma)
    # scheduler_dec = StepLR(feature_extractor_optimizer, step_size=1, gamma=args.gamma)
    scheduler_enc_rnn = StepLR(encoderRNN_optimizer, step_size=3, gamma=args.gamma)
    scheduler_dec = StepLR(decoderRNN_optimizer, step_size=3, gamma=args.gamma)
    ...
    train_files = os.listdir('/home/kasra/kasra_files/data-shenasname/cropped_data1')
    validation_files = os.listdir('/home/kasra/kasra_files/data-shenasname/cropped_validation_files')
    test_files = os.listdir('/home/kasra/kasra_files/data-shenasname/cropped_test')
    # file_test = []
    # for file in files_test:
    #     file_test.append([file, file.split('.')[0]])

    # criterion_mse = nn.MSELoss(reduction='mean')
    criterion_ctc = nn.CTCLoss(blank=127, reduction='mean', zero_infinity=True)
    # mse_loss_min = np.Inf
    ctc_loss_min = np.Inf
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for file in train_files:
            ...
            dataloader1 = DataLoader(dataset1, batch_size=batch_size, shuffle=True, drop_last=True)
            ctc_loss = train(args=args, encoderCNN=encoderCNN, encoderRNN=encoderRNN, decoderRNN=decoderRNN
                             , encoderCNN_optimizer=encoderCNN_optimizer, encoderRNN_optimizer=encoderRNN_optimizer
                             , decoderRNN_optimizer=decoderRNN_optimizer,device=device, dataloader1=dataloader1
                             , epoch=epoch, start=start,  criterion_ctc=criterion_ctc, batch_size= args.batch_size)

        for file in validation_files:
            data_path = '/home/kasra/kasra_files/data-shenasname/cropped_validation_files' + '/' + file
            dataset1 = final_dataLoader(data_file=data_path, transform=trans)
            dataloader1 = DataLoader(dataset1, batch_size=args.valid_batch_size, shuffle=True, drop_last=True)
            ctc_loss_min = evaluation(args, dataloader1, encoderCNN, encoderRNN, decoderRNN
                                   , encoderCNN_optimizer, encoderRNN_optimizer
                                   , decoderRNN_optimizer, device, epoch
                                   , args.valid_batch_size, criterion_ctc, ctc_loss_min)

        scheduler_enc.step()
        scheduler_dec.step()
        scheduler_enc_rnn.step()
