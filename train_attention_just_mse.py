...
import numpy as np
import tqdm
from Networks import Encoder, DecoderRNN, CustomModel_mse, AttnDecoderRNN
...
def train(args, encoder, encoder_optimizer, decoder, decoder_optimizer, device, dataloader1, epoch, start, criterion_mse, criterion_ctc, batch_size):
    ...
    for batch_idx, (images, target) in enumerate(dataloader1):
        ...
        try:
            ...
            if (batch_idx + 1) % args.log_interval == 0 and (batch_idx != 0):
                print('Train Epoch : {} [{}/{} ({:.0f}%)]\tLoss ctc: {:.6f}\t Loss mse: {:.6f}'.format(
        ...
        # torch.cuda.empty_cache()
    print('Epoch {} MSE TRAINING. final losses:\nLoss mse: {:.6f}\t Loss ctc: {:.6f}'.format(
        epoch, tot_loss_mse / (len(dataloader1) * batch_size),
...
#     if val_loss < val_loss_min:
        ...
        if args.save_model and (mse_loss < mse_loss_min):
            ...
            filename = ('/home/kasra/PycharmProjects/Larkimas/model_checkpoints'
                        '/epoch_{0}_mse_loss: {1}.pt').format(epoch, np.round(mse_loss, 4))
            torch.save({'epoch': epoch, 'state_dict encoder': encoder.state_dict(),
                        'encoder_optimizer': encoder_optimizer.state_dict()}, filename)
...
# '/home/kasra/PycharmProjects/Larkimas/model_checkpoints/encoder_epoch_1_ctc_loss_4.3.pt'
    ...
    decoder = AttnDecoderRNN(hidden_size=16, output_size=16).to(device)
